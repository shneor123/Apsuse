...
function getNextBookId(bookId, diff) {
  ...
  else if(nextBookIdx === -1) nextBookIdx = books.length - 1
  return books[nextBookIdx].id
}

function showOptionalBooks(query) {
  const newQuery = query.replace(/\s/g, '%20');
  const url = `https://www.googleapis.com/books/v1/volumes?printType=books&q=${newQuery}`;
  return axios.get(url)
        .then(res => res.data.items)
}

function addBook(bookToSave) {
  let books = _loadBooksFromStorage();
  let newBook = _createBook(bookToSave);
  books = [newBook, ...books];
  _saveBooksToStorage(books);
  return Promise.resolve();
}
